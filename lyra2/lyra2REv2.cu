...
extern void lyra2vc0ban_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNonce, uint64_t *d_outputHash, int order);
extern void lyra2vc0ban_cpu_init(int thr_id, uint32_t threads, uint64_t* d_matrix);
...
extern "C" int scanhash_lyra2vc0ban(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
...
// cleanup
extern "C" void free_lyra2vc0ban(int thr_id)
{
	if (!init[thr_id])
		return;
