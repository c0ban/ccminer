...
void lyra2vc0ban_gpu_hash_32_1(uint32_t threads, uint32_t startNounce, uint2 *outputHash)
{
...
#endif
void lyra2vc0ban_gpu_hash_32_2(uint32_t threads, uint32_t startNounce, uint64_t *outputHash)
{
...
__global__ __launch_bounds__(64, 1)
void lyra2vc0ban_gpu_hash_32_3(uint32_t threads, uint32_t startNounce, uint2 *outputHash)
{
	const uint32_t thread = blockDim.x * blockIdx.x + threadIdx.x;
...
__host__
void lyra2vc0ban_cpu_init(int thr_id, uint32_t threads, uint64_t *d_matrix)
{
...
__host__
void lyra2vc0ban_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint64_t *g_hash, int order)
{
	int dev_id = device_map[thr_id % MAX_GPUS];
	...
	if (cuda_arch[dev_id] < 500)
		cudaFuncSetCacheConfig(lyra2vc0ban_gpu_hash_32_2, cudaFuncCachePreferShared);

	lyra2vc0ban_gpu_hash_32_1 << <grid2, block2 >> > (threads, startNounce, (uint2*)g_hash);

	lyra2vc0ban_gpu_hash_32_2 << <grid1, block1, 48 * sizeof(uint2) * tpb >> > (threads, startNounce, g_hash);

	lyra2vc0ban_gpu_hash_32_3 << <grid2, block2 >> > (threads, startNounce, (uint2*)g_hash);
